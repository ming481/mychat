...
function notifyNewMessage(payload) {
  window.chatApp?.notifyNewMessage?.(payload);
}
  ...
  useEffect(() => {
    ...
    socket.on('message:private', (msg) => {
      ...
      if (isIncomingFromOther) {
        notifyNewMessage({
          title: msg.sender_nickname || '新消息',
          body: msg.type === 0 ? (msg.content || '你收到一条新消息') : '你收到一个文件',
        });
      }
    ...
    socket.on('message:group', (msg) => {
      ...
      if (isIncomingFromOther) {
        notifyNewMessage({
          title: msg.sender_nickname ? `${msg.sender_nickname} 的群消息` : '群聊新消息',
          body: msg.type === 0 ? (msg.content || '你收到一条群消息') : '你收到一个群文件',
        });
      }
